...
import off.kys.amber_lang.transpiler.ast.Parameter
...
import off.kys.amber_lang.transpiler.lexer.Token

/**
 * Internal exception used to unwind the stack to the nearest synchronization point.
 */
...
class Parser(private val tokens: List<Token>, private val filePath: String) {
    ...
    /**
     * Synchronizes the parser state after an error by skipping tokens until
     * it finds a likely start of a new statement.
     */
    private fun synchronize() {
        consume()
        while (peek() !is Token.EOF) {
            // If the previous token was a newline, we are likely at a new statement
            if (tokens.getOrNull(state.currentIndex - 1) is Token.Newline) return

            // Or if we hit a keyword that usually starts a statement
            val token = peek()
    ...
    private fun parsePrimaryExpression(): Expression {
        var expr: Expression = when (val token = consume()) {
            ...
            is Token.Separator if token.value == "(" -> {
                skipNewlines()
        ...
        while (true) {
            when (val next = peek()) {
                is Token.Separator if next.value == "." -> {
                    consume()
    ...
    private fun parseStringLiteral(token: Token.StringLiteral): Expression {
        ...
        while (i < value.length) {
            if (value[i] == '$') {
                if (i > 0 && value[i - 1] == '\\') {
                    // Escaped $, ignore for now and handle in literal
                    i++
                    continue
                }
        ...
        // Convert segments to strings and join them
        return segments.map {
