...
struct AMBER_RESULT __am_fail() {
return __amber_rt_result_error("something went wrong")    ;
}
double __am_res;
int main(int argc, char** argv) {
    GC_INIT();
__am_res = ({ struct AMBER_RESULT __res = __am_fail(); __res.has_error ? ({ __amber_rt_println("Caught error!"); 0.0; }) : __amber_rt_unbox_double(__res.AMBER_VALUE); })    ;
__amber_rt_println(__amber_rt_str_concat("Result is: ", __amber_rt_to_string(__amber_rt_box_double(__am_res))))    ;
    return 0;
}
